...
| "Capture or update governed project work" | Resolve the project, then load `references/work-management.md`; use source issue/PR evidence plus `project_management_inventory` -> reconciliation preview -> explicit idempotent apply. |
| "Update work status or link a new child issue" | Load `references/work-management.md`; update the authoritative source record at the lifecycle transition, link independent follow-up with `github_sub_issue_write` when supported, then reconcile the Project projection. |
...
work. Treat the repository's issue/PR/change/Git evidence as authority and the
shared Project as an operational projection.

For each material work item:

1. search the bounded inventory and source repository first; do not create a
   duplicate record;
2. create or update one complete source issue/PR with every applicable canonical
   field, even when the live Project cannot yet represent that field;
3. inspect live Project schema/options when field behavior matters;
4. preview reconciliation before apply, then apply only with an explicit stable
   idempotency key and current observed revision/evidence;
5. update the source record and Project projection at meaningful lifecycle
   transitions such as active work, review, verification, documentation,
   blocked/hold/deferred state, and completion;
6. when work exposes a separate actionable defect, risk, decision, or follow-up,
   create its own complete source record and link it to the parent with the
   provider's native sub-issue relation when available;
7. do not mark work `Done` merely because implementation ended: preserve any
   required verification, documentation reconciliation, approval, or operator
   hold until its exact completion evidence exists.

Never invent Project fields or status options to hide schema drift. If the live
Project lacks a canonical field, retain that fact in the source record and
project only fields whose current schema has been verified.
...
- Work Management auto-add/status/close synchronization may be disabled. A source
  issue existing on GitHub does not prove the shared Project is current, and a
  partial Project schema does not excuse incomplete canonical source metadata.
...
- when Work Management applies, the authoritative source record is complete,
  meaningful lifecycle transitions and child links are recorded, and the
  currently supported Project projection has been reconciled or its limitation
  is explicit;
