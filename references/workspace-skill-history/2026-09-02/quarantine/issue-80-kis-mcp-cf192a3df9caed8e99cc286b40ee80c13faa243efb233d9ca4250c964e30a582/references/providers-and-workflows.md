...
and specialist-review workflow, reconciles the review branch, and creates one
open non-draft PR at the verified published head. Exact-tree publication is valid
only for a tree-equivalent source base; when the verified remote default has
advanced with different content, current KIS uses an explicit-base three-way
reconciliation that preserves remote-only changes and fails closed on conflicts.
Exact-head CI remains required for the resulting PR head.

The coordinator deliberately stops before merge, branch deletion, worktree
cleanup, auto-merge, release, or default-branch mutation. After an exact
registered merge, use the advertised
`kis_github_refresh_registered_default_branch` to refresh only the registered
`refs/remotes/origin/<default>` tracking ref to exact GitHub truth before remote
review-branch deletion and local worktree cleanup. The same bounded refresh is
required immediately before creating a new governed worktree when the current
workflow advertises that lifecycle step. It must not reset the local default
branch or working tree. If a running instance lacks one of these newer
operations, discover the current safe-closeout workflow and report the runtime
lag rather than substituting generic fetch/reset behavior.
...
repository-owned desired Project projection remains the authority for intended
field/view semantics; the live provider schema remains the invocation contract.

Load `references/work-management.md` for the complete intake, applicable-field,
status-update, sub-issue, reconciliation, verification, and documentation
closeout routine. In particular, source issue/PR metadata stays complete even
when the live Project is missing richer fields, and every Project mutation is
previewed before an explicit idempotent apply.

The `complete-work-managed-pull-request` workflow requires
`project_management_merge_readiness` before the exact-head registered merge and
`project_management_documentation_reconcile` after confirmed merge evidence when
documentation is required. Required work remains non-final until
`post_merge_complete` exists at an exact revision.

The approved GitHub MCP surface may still lack generic Project-field, Status
option-schema, saved-view, or native-workflow provisioning. Report those gaps;
do not bypass them with unrestricted GraphQL. Custom/native automation remains
disabled until separately commissioned.
