...
### Protocol and host interoperability checks

When the failure is about MCP transport or client compatibility rather than KIS
startup alone:

1. run the local transport smoke first to establish the KIS baseline;
2. inspect initialization evidence for the negotiated protocol revision and
   advertised client/server capabilities;
3. exercise `tools/list`, one representative `tools/call`, and the relevant
   error/result shape; add resources/prompts only when the failing path uses them;
4. after tool metadata changes, refresh or reconnect the target client before
   concluding that selection or schema behavior is stale;
5. exercise the actual target host before claiming host or cross-host
   compatibility; local transport success alone is insufficient;
6. when the conclusion depends on current MCP specification semantics, load
   `mcp-development` and verify the current primary protocol sources.
...
Before creating a new governed worktree for a registered GitHub repository, use
the current safe workflow to refresh its registered default-branch tracking ref
to the exact verified GitHub default SHA. When advertised, the bounded operation
is `kis_github_refresh_registered_default_branch`; it updates only
`refs/remotes/origin/<default>` and must not reset the local branch or worktree.
...
After an exact registered merge, first refresh the registered default tracking ref to the verified merged GitHub default SHA. Then cleanup must run from the clean primary checkout:
