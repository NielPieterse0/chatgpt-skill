...
contract. Omit `review_types` when the workflow should select its current
risk-profile/default reviews. Pass an explicit empty list only when no specialist
review is intended and the governing workflow permits that choice. Do not
silently replace an explicit empty list with a manufactured review.

For `source="commit"`, preserve the exact commit source through selection and
execution even when the working tree is clean; commit-derived changed paths are
not interchangeable with working-tree state.

The workflow composes only its fixed selection, verification, and review
operations; it is not an arbitrary command/workflow executor.
