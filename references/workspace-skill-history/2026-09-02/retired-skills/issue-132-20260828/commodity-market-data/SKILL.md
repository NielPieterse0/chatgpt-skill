...
description: Use when acquiring, validating, joining, or versioning commodity or natural-gas market/fundamental data where historical availability, release timing, revisions, futures contracts, rolls, sessions, and term structure must be point-in-time correct.
---
# Commodity Market Data

## Purpose
Extend the shared `data-engineer` and `dataset-auditor` disciplines with commodity-market-specific point-in-time data rules.
...
8. Every derived dataset must carry immutable source identities, hashes, as-of timestamp, vintage identity, and transformation lineage. When the governing repository defines an experiment or data contract, map these fields into that contract rather than inventing a parallel schema.
...
## Completion
A dataset is usable only when a reviewer can reconstruct what information was available, from which contract or vintage, at the stated forecast cutoff.