...
description: >
  Use when MCP work is protocol-centric, cross-host, or review/modernization
  oriented: specification correctness, interoperability, version negotiation,
  transports, authorization, negotiated capabilities, MCP Apps portability, host
  boundaries, or architecture of an existing integration. Use it to decide what
  belongs to portable MCP versus a host extension. Prefer openai-mcp-server for
  straightforward ChatGPT/Codex server construction, openai-mcp-app-ui for
  OpenAI-facing interactive UI, and mcpb-local-packaging only for Anthropic MCPB
  local distribution. Do not use for ordinary APIs or generic frontend work.
---
...
This skill owns MCP protocol and integration method, not repository governance.
Before changing a repository, determine its declared workflow authority.

- If the repository is KIS-managed, load the `kis-mcp` skill and use KIS for
  project/work tracking, governed repository and GitHub actions, verification,
  review, integration, and hygiene. Treat KIS results as operational authority.
- Keep MCP design decisions inside this skill, but do not bypass repository
  policy with direct Git/GitHub/process actions when a governed route exists.
- If KIS is not the declared authority, follow the repository's native workflow
