...
Pair this with accurate tool annotations — `readOnlyHint: true` on every read tool and `destructiveHint: true` on delete/overwrite tools. Treat annotations as host/model-selection metadata, not authorization; enforce permissions in the server.

If you ship write/delete, preserve explicit user control using a capability-supported confirmation mechanism. For OpenAI-targeted interactive confirmation UI, route the UI concern to `openai-mcp-app-ui`; for protocol/cross-host confirmation semantics, route to `mcp-development`.
